...
import com.nttdata.yankiaccount.entity.dto.Maestros;
import com.nttdata.yankiaccount.entity.dto.MonederoTransaction;
import com.nttdata.yankiaccount.redis.MonederoRedisService;
...
public class MonederoAccountController {
  ...
  @Autowired
  IMonederoAccountService service;
  ...
  public void consumeMessage(MonederoTransaction monederoTransaction) {
    ...
    service.findByNroPhone(monederoTransaction.getDestinationNumber()).next()
        .flatMap(ac -> {
  ...
  @PostMapping("/create")
  public Mono<ResponseEntity<MonederoAccount>> create(
      @RequestBody MonederoAccount monederoAccount) {
    log.info("Buscando ....");
    return service.findByNroPhone(monederoAccount.getNroPhone()).collectList()
        .flatMap(c -> {
          if (c.isEmpty()) {
  ...
  @PutMapping("/update")
  public Mono<ResponseEntity<MonederoAccount>> update(
      @RequestBody MonederoAccount monederoAccount) {
    log.info("buscando tarjeta de credito");
    return service.findById(monederoAccount.getId()).flatMap(cc -> {
